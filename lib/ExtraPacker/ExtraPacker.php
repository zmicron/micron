<?php
    ...
    require_once dirname(__FILE__) . "/Lib/JsPacker/class.JavaScriptPacker.php";
    ...
    class ExtraPacker
    {
        ...
        private $flgJSSimpleCompress     = true;
        ...
        const ONLY_MERGE_CSS = 0;
        const ONLY_MERGE_JS  = 0;
        ...
        private function HtmlPack($_content)
        {
            $_content = HtmlPacker::Pack($_content);
            return $_content;
        }
        ...
        public static function MemInBytyes($mem)
        {
        ...
        private function WriteDataInCache($type, $arrFiles, $arrExeptions = array())
        {
            ...
            global $g_config;
            $memInBytes = self::MemInBytyes($g_config['phpIni']['memory_limit']);
            ...
            if ($type == 'js')
            {
                ...
                {
                    $jsPacker = new JavaScriptPacker($fullContent, ($this->flgJSSimpleCompress ? 'None' : 'Normal'), true, false);
            ...
            $packed   = $packed . $fullExeptionContent;
        ...
        private function JsPack($_content)
        {
            ...
            foreach ($m[1] as $k => $jsAddr)
            {
                ...
                $_content     = str_ireplace($m[0][$k], "", $_content);
            ...
            $_content = str_ireplace(self::TAG_EXTRAPACKER, self::TAG_EXTRAPACKER . '<script type="text/javascript" src="' . $inc . '"></script>', $_content);
        ...
        private function CssPack($_content)
        {
            ...
            $_content = str_ireplace(self::TAG_EXTRAPACKER, self::TAG_EXTRAPACKER . '<link rel="stylesheet" type="text/css" href="' . $inc . '" />', $_content);
